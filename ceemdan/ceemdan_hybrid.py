...
import os
import warnings
import hashlib  # ← ДОБАВЛЕНО: для детерминированного хэша
import numpy as np
import pandas as pd
...
# Настройка потоков — выполняется до импорта тяжелых библиотек
...
# =============================================================================
#  ИМПОРТЫ НА УРОВНЕ МОДУЛЯ (исправляет 'module object is not callable')
# =============================================================================
def check_ceemdan() -> bool:
    try:
        from PyEMD import CEEMDAN
        # Проверяем, что класс действительно вызываемый
...
# Импортируем класс сразу, чтобы избежать проблем с локальными импортами
...
# =============================================================================
#  ФУНКЦИЯ ПОЛУЧЕНИЯ IMF (исправлен кэш и способ декомпозиции)
# =============================================================================
def _get_imfs(
    ...
    # ← ИСПРАВЛЕНИЕ 1: Детерминированный ключ кэша + учет параметров
    ...
    with warnings.catch_warnings():
        ...
        # ← ИСПРАВЛЕНИЕ 2: Передаём random_state для воспроизводимости
        ...
        # ← ИСПРАВЛЕНИЕ 3: Универсальный вызов декомпозиции (API 1.9.0+)
        try:
            # Новый API: метод .decompose()
            if hasattr(cem, 'decompose'):
                imfs = cem.decompose(train_values.astype(float))
            # Старый API: вызов экземпляра
            else:
                imfs = cem(train_values.astype(float))
        except TypeError as e:
            if "'module' object is not callable" in str(e):
                # Фолбэк на метод, если прямой вызов не сработал
                imfs = cem.decompose(train_values.astype(float))
            else:
                raise

    # ← ИСПРАВЛЕНИЕ 4: Корректное управление кэшем (LRU-style)
...
def _prophet_imf(s: pd.Series, test_size: int, verbose: bool = False) -> Tuple[np.ndarray, dict]:
    try:
        ...
        # ← ИСПРАВЛЕНИЕ 5: Используем реальный индекс вместо фиктивных дат
        # Если индекс не datetime — создаём корректный range
        if isinstance(s.index, pd.DatetimeIndex):
            train_dates = s.index
            freq = pd.infer_freq(s.index) or "MS"
        else:
            # Fallback для числовых индексов
...
def ceemdan_hybrid_forecast(
    ...
    noise_width: float = 0.05,
    verbose: bool = False,
    random_state: Optional[int] = 42,  # ← ИСПРАВЛЕНИЕ 6: Параметр воспроизводимости
    ...
    imfs = _get_imfs(
        ...
        random_state=random_state  # ← Передаём random_state
    )
    if verbose:
    ...
    imf_forecasts = []
    fallback_count = 0
    MIN_IMF_LENGTH = test_size + 12  # ← Именованная константа вместо магического числа
...
def ceemdan_arima(series, test_size=18, ceemdan_trials=50, random_state=42, **kw):
    return ceemdan_hybrid_forecast(
        ...
        random_state=random_state,  # ← Передаём дальше
...
def ceemdan_es(series, test_size=18, ceemdan_trials=50, random_state=42, **kw):  # ← ИСПРАВЛЕНИЕ 7: Переименовано
    """Гибрид CEEMDAN + Exponential Smoothing (корректное имя)"""
    return ceemdan_hybrid_forecast(
        series, "ES", "CEEMDAN+ES",  # ← Честное имя модели в метриках
        test_size, ceemdan_trials,
        random_state=random_state,
        verbose=kw.get("verbose", False)
    )

# ← ИСПРАВЛЕНИЕ 8: Оставляем старое имя как алиас с предупреждением
def ceemdan_lstm(series, test_size=18, ceemdan_trials=50, random_state=42, **kw):
    """
    ⚠️ DEPRECATED: Эта функция использует Exponential Smoothing, а не LSTM.
    Используйте ceemdan_es() для ясности.
    """
    import warnings
    warnings.warn(
        "ceemdan_lstm() использует Exponential Smoothing, а не LSTM. "
        "Для корректных результатов используйте ceemdan_es().",
        UserWarning,
        stacklevel=2
    )
    return ceemdan_es(
        series, test_size, ceemdan_trials, random_state, **kw
    )